...
| `@acme/types/seed` | [`SeedContext<TTx>`](./src/seed.ts), [`SeedRun<TTx>`](./src/seed.ts), re-exported `DeployStage` — used by Drizzle seed modules under `packages/db/scripts/seeds/` |
...
// Derive `tx` from your DB client — see packages/db `scripts/seeds/*.ts`
