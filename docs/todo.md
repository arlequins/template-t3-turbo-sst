...
- [ ] Add a local PostgreSQL `compose.yml` and `db:start` / `db:stop` commands for a zero-setup development path.
- [ ] Add `DATABASE_SSL_MODE=disable|require|verify-full` instead of forcing one TLS behavior for every environment.
- [ ] Run migration, seed idempotency, and representative tRPC CRUD integration tests against PostgreSQL in CI.
